...
expr *make_call(expr *lv, expr *reversed_elist, unsigned line, SymbolTableEntry *tempvar) {
    // expr *emit_iftableitem(expr *e, SymbolTableEntry *temp, SymTable *symtable, scopeLists *lists, unsigned int scope, unsigned int line) {
    ...
    expr *result = create_expr(var_e, tempvar, NULL, 0.0f, NULL, '\0');
    emit(getretval, NULL, NULL, NULL, 0U, line);
    return result;
}
